...
def knapsack_bottom_up(weights, values, capacity):
    ...
    :rtype: int
    """
    dp = np.zeros((len(weights) + 1, capacity + 1), dtype=int)

    for i in range(1, len(weights) + 1):        # for each item i 
        for j in range(0, capacity + 1):        # for each capacity j 
            exclude_item = dp[i - 1][j]         # on't take item i
            if weights[i - 1] <= j:
                include_item = values[i - 1] + dp[i - 1][j - weights[i - 1]]        # take item i 
            else:
                include_item = 0
