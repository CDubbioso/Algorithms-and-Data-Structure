...
class BinarySearch():
    ...
    def step(self, min_index, max_index):
        ...
        :rtype: int
        """
        # Base case: search space is empty -> value not in list
        if min_index > max_index:
            return None

        # Pick the middle index of the current search space
        mid_index = (min_index + max_index) // 2
        mid_value = self.sorted_list[mid_index]

        # Found it
        if mid_value == self.value:
            return mid_index

        # Value must be in the right half
        if mid_value < self.value:
            return self.step(mid_index + 1, max_index)

        # Value must be in the left half
        return self.step(min_index, mid_index - 1)
